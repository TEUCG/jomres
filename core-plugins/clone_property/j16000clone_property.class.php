...
class j16000clone_property
	...
	function clone_me_now($property_uid,$new_property_name)
		...
		$jomresTablesArray['__jomres_custom_text']		=array('autoid'=>'uid','idkey'=>'property_uid','columns'=>array('uid','constant','customtext','language','reserved', 'language_context') );
		...
		foreach ($jomresTablesArray as $key=>$val)
			...
			if (!in_array($key,$this->tablesToSkipUpdateOn) )
				...
				foreach ($selectResults as $res)
					{
					if ($key == '__jomres_custom_text' && $res->constant == '_JOMRES_CUSTOMTEXT_PROPERTY_NAME' )
						{
