...
class j06002list_tariffs_micromanage
	...
	function __construct()
		...
		foreach ($basic_rate_details->rates as $roomclass_uid => $t) {
			foreach ($t as $tarifftype_id => $r) {
				...
				if (!using_bootstrap())
					...
					$jrtb .= $jrtbar->toolbarItem('edit',jomresURL(JOMRES_SITEPAGE_URL."&task=edit_tariff_micromanage&tarifftypeid=".$tarifftype_id ),'');
					$jrtb .= $jrtbar->toolbarItem('copy',jomresURL(JOMRES_SITEPAGE_URL."&task=edit_tariff_micromanage&tarifftypeid=".$tarifftype_id."&clone=1"),'');
					$jrtb .= $jrtbar->toolbarItem('delete',jomresURL(JOMRES_SITEPAGE_URL."&task=delete_tariff_micromanage&tarifftypeid=".$tarifftype_id ),'');
					...
					{
					$toolbar->newToolbar();
					$toolbar->addItem( 'icon-edit', 'btn btn-info', '', jomresURL( JOMRES_SITEPAGE_URL . '&task=edit_tariff_micromanage' . '&tarifftypeid=' . $tarifftype_id ), jr_gettext( 'COMMON_EDIT', 'COMMON_EDIT', false ) );
					$toolbar->addSecondaryItem( 'icon-copy', '', '', jomresURL( JOMRES_SITEPAGE_URL . '&task=edit_tariff_micromanage' . '&tarifftypeid=' . $tarifftype_id . "&clone=1" ), jr_gettext( '_JOMRES_COM_MR_LISTTARIFF_LINKTEXTCLONE', '_JOMRES_COM_MR_LISTTARIFF_LINKTEXTCLONE', false ) );
					$toolbar->addSecondaryItem( 'icon-trash', '', '', jomresURL( JOMRES_SITEPAGE_URL . '&task=delete_tariff_micromanage' . '&tarifftypeid=' . $tarifftype_id ), jr_gettext( 'COMMON_DELETE', 'COMMON_DELETE', false ) );
		...
		$output['_JOMRES_MICROMANAGE_MULTIPLE_TARIFFS_LIST_PAGE']	= jr_gettext('_JOMRES_MICROMANAGE_MULTIPLE_TARIFFS_LIST_PAGE','_JOMRES_MICROMANAGE_MULTIPLE_TARIFFS_LIST_PAGE',false);
