...
class j06001dashboard extends jomres_dashboard
	...
	function dashboardMakeMonthList( $orientation = "" )
		...
		for ( $i = 1; $i <= $this->monthsToShow; $i++ )
			{
			$link           = jomresURL( JOMRES_SITEPAGE_URL_NOSEF . '&requestedMonth=' . $nm );
		...
		$dropdown = jomresHTML::selectList( $monthsArray, 'requestedMonth', 'size="1" onchange="location.href=dashboardmonthdropdown.requestedMonth.options[selectedIndex].value" ', 'value', 'text', jomresURL( JOMRES_SITEPAGE_URL_NOSEF . '&requestedMonth=' . $this->requestedMonth ) );
