...
class j03120send_email_guest_enquiryapproval
	{
	function __construct( $componentArgs )
		...
		$email_type = 'email_guest_enquiryapproval';
		...
		if ( $email_when_done )
			...
			if ( !jomresMailer( $booking_email_details->data[$contract_uid]['PROPERTY_EMAIL'], 
				...
				error_logging( 'Failure in sending enquiry approval email to guest. Target address: ' . $booking_email_details->data[$contract_uid]['EMAIL'] . ' Subject ' . $booking_email_details->parsed_email['subject'].$booking_email_details->parsed_email['text'] );
			}
		else
