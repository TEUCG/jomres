...
class j03100send_email_hotel_newenquiry
	{
	function __construct( $componentArgs )
		...
		$email_type = 'email_hotel_newenquiry';
		...
		if ( $email_when_done )
			...
			if ( !jomresMailer( $booking_email_details->data[$contract_uid]['EMAIL'], 
				...
				error_logging( 'Failure in sending new enquiry email to hotel. Target address: ' . $booking_email_details->data[$contract_uid]['PROPERTY_EMAIL'] . ' Subject' . $booking_email_details->parsed_email['subject'].$booking_email_details->parsed_email['text'] );
			}
		else
